...
public class CLI {
	...
	public CLI(String[] args){
		...
		options.addOption("pmin", true, "The pmin value (double > 0)");
		options.addOption("pmax", true, "The pmax value (double > 0)");
	...
	public void parse() {
		...
		try {
			...
			if (cmd.hasOption("pmin")){
				try{
					setPmin(Double.parseDouble(cmd.getOptionValue("pmin")));
			...
			if (cmd.hasOption("pmax")){
				try{
					setPmax(Double.parseDouble(cmd.getOptionValue("pmax")));
				} catch(NumberFormatException e){
					help();
				}
			}else {
